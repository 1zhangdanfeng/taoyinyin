    	...
    	if (IS_POST && I('post.query') && I('post.value')) {
            ...
            switch($query){
                ...
                case 'mobile':
                    ...
                    $count = M('order')->where($where)->count();
                    $page_obj = new \Think\Page($count, $limit);
                    $qme = M('order')->where($where)->page($page.','.$limit);
                    break;
            }
        }else{
            $count = M('order')->count();
            $page_obj = new \Think\Page($count, $limit);
            $qme = M('order')->page($page.','.$limit);
        }
        $result = $qme->order('time desc')->select();
        $shops = array();
        foreach ($result as $key => $row) {
            $result[$key]['time'] = $this->date($row['time']);
            if($row['order_sid'] > 0) $shops[] = $row['order_sid'];
        }
        $shops = D('order')->getShops($shops);
