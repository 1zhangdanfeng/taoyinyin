...
class UserController extends BaseController {
	...
	public function lists()
	{
		...
		if ($uname) {
			$user = M('user')->where(array('uname' => array('LIKE', "%{$uname}%")))->limit(($page - 1) * $limit, $limit)->select();
			$count = M('user')->where(array('uname' => array('LIKE', "%{$uname}%")))->count();
			$page_obj       = new \Think\Page($count, $limit);
		}else{
			$user = M('user')->limit(($page - 1) * $limit, $limit)->order('regtime desc')->select();
			$page_obj       = new \Think\Page($this->count, $limit);
		}

		$schools = D('order')->getSchools();
		foreach ($user as $key => $value) {
			$user[$key]['school'] = $schools[$user[$key]['school']];
		}
